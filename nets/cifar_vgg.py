...
class VGG(nn.Module):
  ...
  def apply_layer_weight(self, layer_name, keep_inds, weight):
    if keep_inds is not None:
      ...
      # don't need to prune weight in the next layer
      # because the mask is multiplied after bn
    else:
      ...
      if not isinstance(layer, standard_block):
        ...
        if ind < self.name_to_ind[layer_name]:
          ...
          if sample_inds is not None:
            ...
            patches = x.detach()
          # return conv input patches, prev_prunable, identity, shortcut_patches
...
def vgg16(num_classes=10):
  return VGG(conv_config=[64, 64, 'M',
                          128, 128, 'M',
                          256, 256, 256, 'M',
                          512, 512, 512, 'M',
                          512, 512, 512, 'M'],
...
#   net = vgg16()
#
#   # net.apply_actions(np.random.rand(13, 1))
