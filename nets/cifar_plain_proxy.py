...
class PLAIN_Proxy:
  ...
  def compress(self, action):
    ...
    real_a = np.minimum(action, max_a)
    if self.lower_bound:
      real_a = np.maximum(real_a, min_a)
...
def plain8_proxy(lim_type, ratio, min_action, lower_bound=True):
  return PLAIN_Proxy(conv_config=[16, 16, 'M', 32, 32, 'M', 64, 64, 'M'],
                     lim_type=lim_type,
                     ratio=ratio,
...
def plain20_proxy(lim_type, ratio, min_action, lower_bound=True):
  return PLAIN_Proxy(conv_config=[16, 16, 16, 16, 16, 16, 'M',
                                  32, 32, 32, 32, 32, 32, 'M',
                                  64, 64, 64, 64, 64, 64, 'M'],
                     lim_type=lim_type,
                     ratio=ratio,
...
#
#   proxy = plain8_proxy(lim_type='flops',
#                        ratio=0.3,
#                        min_action=0.1,
#                        lower_bound=False)
...
#   while not done:
#     next_state, action, done = proxy.compress(0.9)
